<template>
  <view style="width: 100%; height: 100vh; box-sizing: border-box">
    <!-- 顶部导航 -->
    <CustomHeader :title="'6半全'" :showBack="true" :isIndex="false" :showIcon="false" :isSelected="false" :selectedPlay="''" @funnel-click="handleFunnel" @back-click="onBackClick" />
    <!-- 🌟 新增：期数选择组件（和4球父组件一致） -->
    ...
    <BetBar :min-match-count="6" title="6场" :show-clear-btn="true" :confirmBtnEnabled="true" :confirm-btn-enabled="selectedMatchCount >= 6" :selected-count="selectedMatchCount" @clear="handleClearAll" @confirm="goToSchemeEdit" />
...
import List from "@/pages/sixIndex/list.vue";
...
export default {
  ...
  methods: {
    ...
    handleClearAll() {
      this.drawerList.forEach((drawer, drawerIdx) => {
        drawer.lotteryList.forEach((item, matchIdx) => {
          this.$set(drawer.lotteryList[matchIdx], "halfHomeSelected", false);
          this.$set(drawer.lotteryList[matchIdx], "halfVsSelected", false);
          this.$set(drawer.lotteryList[matchIdx], "halfAwaySelected", false);
          this.$set(drawer.lotteryList[matchIdx], "fullHomeSelected", false);
          this.$set(drawer.lotteryList[matchIdx], "fullVsSelected", false);
          this.$set(drawer.lotteryList[matchIdx], "fullAwaySelected", false);
        });
      });
      uni.showToast({
        title: "已清空选择",
    ...
    syncUpdatedMatches(updatedData) {
      ...
      this.drawerList.forEach((drawer) => {
        drawer.lotteryList.forEach((item) => {
          this.$set(item, "halfHomeSelected", false);
          this.$set(item, "halfVsSelected", false);
          this.$set(item, "halfAwaySelected", false);
          this.$set(item, "fullHomeSelected", false);
          this.$set(item, "fullVsSelected", false);
          this.$set(item, "fullAwaySelected", false);
        });
      });
      updatedData.matches.forEach((updatedItem) => {
        this.drawerList.forEach((drawer) => {
          ...
          if (targetMatchIdx !== -1) {
            this.$set(drawer.lotteryList[targetMatchIdx], "halfHomeSelected", updatedItem.halfHomeSelected || false);
            this.$set(drawer.lotteryList[targetMatchIdx], "halfVsSelected", updatedItem.halfVsSelected || false);
            this.$set(drawer.lotteryList[targetMatchIdx], "halfAwaySelected", updatedItem.halfAwaySelected || false);
            this.$set(drawer.lotteryList[targetMatchIdx], "fullHomeSelected", updatedItem.fullHomeSelected || false);
            this.$set(drawer.lotteryList[targetMatchIdx], "fullVsSelected", updatedItem.fullVsSelected || false);
            this.$set(drawer.lotteryList[targetMatchIdx], "fullAwaySelected", updatedItem.fullAwaySelected || false);
          }
        });
      });
    },
    ...
    async loadMatchData(drawNum = "") {
      try {
        ...
        this.showLoading();
        // 请求参数：传递期数，兼容无传入的情况
        const reqParams = { playMethod: 6 };

        // 1. 请求期数列表并过滤空值（和4球一致）
        const resNum = await footballLotteryTraditionDrawNum({ playMethod: 6 });
...
// 🌟 修改scroll-view样式，对齐4球的样式（删除原有多端适配的top/bottom，改用动态绑定）
.match-scroll {
  touch-action: pan-y;
