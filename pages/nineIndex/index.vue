<template>
  <view style="width: 100%; height: 100vh; box-sizing: border-box">
    <!-- 顶部导航 -->
    <CustomHeader :title="'任9'" :showBack="true" :isIndex="false" :showIcon="false" :isSelected="false" :selectedPlay="''" @funnel-click="handleFunnel" @back-click="onBackClick" />
    <!-- 🌟 新增：期数选择组件（和4球/6球一致） -->
    ...
    <BetBar :min-match-count="9" title="9场" :show-clear-btn="true" :confirmBtnEnabled="true" :confirm-btn-enabled="selectedMatchCount >= 9" :selected-count="selectedMatchCount" @clear="handleClearAll" @confirm="goToSchemeEdit" />
...
export default {
  ...
  data() {
    return {
      ...
      currentDrawNum: "",
      drawNumList: [],
      navBarTotalHeight: 88, // 兜底默认值，和4球/6球一致
    ...
    async goToSchemeEdit() {
      try {
        ...
        if (totalSelectedCount < 9) {
          uni.showToast({ title: "请至少选择9场赛事", icon: "none" });
          return;
        }
...
// 🌟 修改scroll-view样式，对齐4球/6球（删除原有多端适配，改用动态绑定）
.match-scroll {
  touch-action: pan-y;
