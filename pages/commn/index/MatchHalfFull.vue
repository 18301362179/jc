  ...
  <view class="match-list" :style="{ paddingTop: paddingTopVal + 'rpx' }">
          ...
          <view class="main-content-row">
            ...
            <view class="main-right">
              <view class="top-right">
                <view class="team-vs">
                  {{ item.home_name }} VS {{ item.visiting_name }}
                </view>
    ...
    <view class="score-popup" v-show="isPopupShow">
      <view class="popup-title">
        {{ currentMatch && currentMatch.home_name }}(主) VS {{ currentMatch && currentMatch.visiting_name }}(客)
      </view>
      ...
      <view class="half-full-container">
        ...
        <view class="half-full-grid">
          <view 
            v-for="(item, idx) in halfFullOptions" 
            :key="idx"
            class="half-full-option"
            :class="{ selected: selectedScores.includes(item.value) }"
            @click="toggleScore(item.value)"
          >
...
export default {
  ...
  computed: {
    ...
    selectedMatchCount() { // 完全保留你原代码，无修改
      let count = 0;
  ...
  watch: {
    finalDrawerList(newVal) { // 完全保留你原代码，无修改
  ...
  return Math.round((px / this.windowWidth) * 750 + 0.5);
    ...
    confirmSelection() {
      ...
      if (targetDrawerIdx > -1 && targetItemIdx > -1) {
        this.$set(
          this.finalDrawerList[targetDrawerIdx].lotteryList[targetItemIdx],
          'selectedScores',
          [...this.selectedScores]
        );
