...
export default {
  ...
  methods: {
    ...
    handleSpfMultiClick(item, spfType) {
      ...
      // 新增：胜平负类型额外判断赔率值
      const isSpfType = ['home_0', 'draw_0', 'away_0'].includes(spfType);
      if (isSpfType) {
        const multiplierKey = {
...
// 各板块标签背景色
.bifen-label { background: #6FBDBD; }
.spf-label { background:#6FBDBD; }
.rspf-label { background: #6AB284; }
.zjq-label { background: #6AB284; }
.bqc-label { background: #6FBDBD; }
