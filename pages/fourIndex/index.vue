<template>
  <view style="width: 100%; height: 100vh; box-sizing: border-box">
    <!-- 顶部导航 -->
    <CustomHeader :title="'4进'" :showBack="true" :isIndex="false" :showIcon="false" :isSelected="false" :selectedPlay="''" @funnel-click="handleFunnel" @back-click="onBackClick" />
    <!-- 期数选择组件 -->
    <DrawNumSelector :draw-num-list="drawNumList" :current-draw-num="currentDrawNum" :nav-bar-total-height="navBarTotalHeight" :title="title" @draw-num-change="onDrawNumChange" />
    ...
    <!-- 底部投注栏组件 -->
    <BetBar :min-match-count="4" title="4进" :show-clear-btn="true" :confirmBtnEnabled="true" :confirm-btn-enabled="selectedMatchCount >= 4" :selected-count="selectedMatchCount" @clear="handleClearAll" @confirm="goToSchemeEdit" />
        ...
        function (updatedItem) {
          this.drawerList.forEach(
            function (drawer, drawerIdx) {
              ...
              if (targetMatchIdx !== -1) {
                // 确保是数组格式
                const homeSelections = Array.isArray(updatedItem.homeScoreSelected) ? updatedItem.homeScoreSelected : [];
                const awaySelections = Array.isArray(updatedItem.awayScoreSelected) ? updatedItem.awayScoreSelected : [];
