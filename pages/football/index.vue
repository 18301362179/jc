<template>
  ...
  <view style="width: 100%; height: 100vh; box-sizing: border-box">
   ...
   <scroll-view class="tab-bar-sticky" scroll-x show-scrollbar="false"  scroll-with-animation>
    ...
    <scroll-view class="match-scroll" scroll-y>
      <!-- 原有玩法组件 -->
      <MatchSpf ref="spfRef" v-if="currentPlay === '胜平负'" :drawer-list="drawerList" :status-bar-height="statusBarHeight + 45 " :header-height="headerHeight || statusBarHeight + 88" @toggle-select="toggleSelect" :go-to-ai-analysis="goToAiAnalysis" />
      <MatchHandicap ref="handicapRef" v-else-if="currentPlay === '让球胜平负'" :drawer-list="drawerList" :status-bar-height="statusBarHeight + 45" @toggle-select="toggleSelect" :go-to-ai-analysis="goToAiAnalysis" />
      <MatchTotalGoals ref="goalsRef" v-else-if="currentPlay === '总进球'" :drawer-list="drawerList" :status-bar-height="statusBarHeight + 45" @toggle-goal-select="toggleGoalSelect" :go-to-ai-analysis="goToAiAnalysis" />
      <MatchHalfFull ref="halfFullRef" v-else-if="currentPlay === '半全场'" :drawer-list="drawerList" :status-bar-height="statusBarHeight + 45" @on-half-full-selected="handleHalfFullSelected" :go-to-ai-analysis="goToAiAnalysis" />
      <MatchScore ref="scoreRef" v-else-if="currentPlay === '比分'" :drawer-list="drawerList" :status-bar-height="statusBarHeight + 45" :toggle-score-select="toggleScoreSelect" @on-score-selected="handleScoreSelected" :go-to-ai-analysis="goToAiAnalysis" />
  ...
  computed: {
    ...
    hasSingleMatch() {
      ...
      if (this.currentPlay === "混合过关") {
        // 混合过关单独处理
        if (this.currentPlay === "混合过关") {
          return this.drawerList.some((drawer) => drawer.lotteryList.some((match) =>  match.betRows.some((row) => row.items.some((item) => item.isSelected))));
        }
      }
  ...
  methods: {
    ...
    handleConfirmMixedSelect(confirmData) {
      try {
        ...
        // 遍历列表，替换更新后的那场数据
        this.drawerList.forEach((drawer) => {
          const idx = drawer.lotteryList.findIndex((m) => m.id === confirmData.id);
          if (idx !== -1) {
            this.$set(drawer.lotteryList, idx, confirmData.updatedItem);
          }
        });
