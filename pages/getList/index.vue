<template>
  <view class="get-list-container">
    ...
    <!-- 把 scroll-view 改成普通 view，内容不够就不会滚动 -->
    <view class="list-scroll" :style="{ paddingTop: 30 + 'px' }">
      <no-data v-if="coinRecordList.length === 0" />

      <view class="list-item" v-for="(item, index) in coinRecordList" :key="index">
        <view class="item-col type-col">{{ item.trade_no || '' }}</view>
        <view class="item-col desc-col">{{ item.payment || '' }}</view>
        <view class="item-col num-col">{{ item.coin_sum || 0 }}</view>
        <view class="item-col time-col">{{ item.update_time || '' }}</view>
      </view>
    </view>
  </view>
</template>
