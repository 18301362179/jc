...
var _default2 = {
  ...
  methods: {
    ...
    handleScoreToggle: function handleScoreToggle(plate, value) {
      console.log(plate, value, 'liangge');
      try {
        // 2. 停售/加载中判断日志
        if (this.isLoading) {
          return;
    ...
    openScorePopup: function openScorePopup(match) {
      ...
      if (Array.isArray(match.selectedSpf) && match.selectedSpf.length > 0) {
        match.selectedSpf.forEach(function (val) {
          var key = _this.reverseSpfMapping[val];
          if (key) {
      ...
      (0, _demo.queryHhgg)({
        ...
        if (res && res.data) {
          ...
          Object.keys(res.data.data1 || {}).forEach(function (key) {
            _this.$set(_this.currentMatch, key, res.data.data1[key]);
          });
          if (res.data.data2) {
            Object.keys(res.data.data2).forEach(function (key2) {
              _this.$set(_this.currentMatch.data2, key2, res.data.data2[key2]);
            });
          }
        }
      }).catch(function (err) {
      ...
      try {
        ...
        this.finalDrawerList.some(function (drawer) {
          return drawer.lotteryList.some(function (item) {
            if (item.id === _this3.currentMatch.id) {
              targetItem = item;
              return true;
            }
            return false;
        ...
        updatedItem.selectedSpf = [].concat((0, _toConsumableArray2.default)(mappedSpf), (0, _toConsumableArray2.default)(mappedRspf));
        ...
        this.finalDrawerList.forEach(function (drawer, di) {
          drawer.lotteryList.forEach(function (item, ii) {
            if (item.id === _this3.currentMatch.id) {
              _this3.$set(drawer.lotteryList, ii, updatedItem);
            }
          });
        });
