...
var _default = {
  ...
  methods: {
    ...
    handleConfirmMixedSelect: function handleConfirmMixedSelect(confirmData) {
      var _this4 = this;
      try {
        // 子组件传过来的就是更新后的单场数据，不需要 finalDrawerList
        if (!confirmData || !confirmData.updatedItem) {
          return;
        }

        // 遍历列表，替换更新后的那场数据
        this.drawerList.forEach(function (drawer) {
          var idx = drawer.lotteryList.findIndex(function (m) {
            return m.id === confirmData.id;
          });
          if (idx !== -1) {
            _this4.$set(drawer.lotteryList, idx, confirmData.updatedItem);
          }
        });
